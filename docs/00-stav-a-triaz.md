...
- Fáza / TRL: **M0 uzavreté, Gate G0 prejdený (2026-06-11)** — `docs/spec.md` v1.0 schválený, jediný zdroj pravdy pre implementáciu. Žiadny tréning neprebehol. Žiadne meranie neexistuje. Ďalej: Zenodo DOI technical note (stupeň 1) → M1 implementácia.
...
- **T1 (= M0, zostávajúce):** ✔ spec.md v1 schválený, **Gate G0 prejdený (2026-06-11)**. ✔ T2-subset + novelty sken (2026-06-11). ✔ draft `paper/technical-note.md` + review kolo 1 (V1–V4, note v0.2). Zostáva: Danielovo finálne čítanie note v0.2 → Zenodo upload (závislé od T4: účet/ORCID) → DOI → D-log zápis. Okno sken→upload < 1 týždeň (do ~2026-06-18).
