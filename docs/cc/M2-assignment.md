# Zadanie pre CC — M2: SSRA v2 training runs (S1/S2)

**Version:** v1.1 (2026-07-12; compute environment GCP → RunPod per D-log 2026-06-17 + 2026-07-12; phases, gates, APs, and configs unchanged) · originally v1 (2026-06-12) · **Milestone:** M2 (`docs/03-poc-plan.md`) · **Mode:** veto-based — items marked AP-x are protocol details introduced by this assignment and stand unless Daniel vetoes them. AP numbering continues from M1 (AP-1…AP-7); AP-1…AP-7 remain in force where applicable.
...
**Environment (updated by D-log 2026-06-17 + 2026-07-12; supersedes the 2026-06-12 GCP L4 plan):** Compute = **RunPod** (per-hour GPU rental; GCP GPU quota confirmed dead end, D-log 2026-06-17). HW default for Phase 1 calibration = **1× A100 80 GB**; availability is checked live in the RunPod console on deploy day, fallback order **A100 PCIe → A100 SXM → H100 PCIe** (speed escalation; AP-12 EUR projection before launch) → **L40S / RTX 4090 / L4** (low-cost fallback). Final HW for Phases 2+ is Daniel's decision on calibration tok/s numbers. Prices verified 2026-07-12 (runpod.io/pricing, USD, per-second billing): L4 $0.39/hr · RTX 4090 $0.69 · A100 PCIe $1.39 · A100 SXM $1.49 · H100 PCIe $2.89; re-verify in the console on deploy day (Pravidlo W). Community tier is admissible (cheaper, preemptible — covered by AP-11); exact Community rates are visible only in the deploy console. Artifacts/checkpoints remain on **GCS `gs://ssra-poc-ew3`** via SA `ssra-runpod@ssra-poc.iam.gserviceaccount.com` (key outside the repo — never in configs, logs, or commits; injection into the pod is a separate assignment). Budget cap **300 EUR cumulative** stands; RunPod bills in USD — fix the EUR/USD rate (ECB reference) on credit top-up day for the AP-12 cost ledger. AP-10 (GCE Spot vs Vertex) is void. Phase 0 is complete (2026-06-14); Phases 1+ are unblocked.
