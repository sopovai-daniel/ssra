...
Mixed key types (RoPE-rotated query attending NoPE summary keys) follow the memory/sink-token precedent (#20) [K — verify in T2]. Contingency config `summary_pos ∈ {none (default), virtual}` where `virtual` rotates every summary key at the fixed virtual position t−w−1 (constant phase wrt query); flip only if T2 verification or M1 smoke surfaces a problem — do not enable silently.
...
*(References #n = `docs/02-prior-art-mapa.md`; [K] items there must pass T2 verification before citation.)*
