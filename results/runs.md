...
| m2-core-flat-s2-850m-lr6e4 | 2026-07-16 (pre-run row) | `experiments/m2-core-flat-s2-850m-lr6e4.yaml` (commit 3db45ef) | flat pre-norm Transformer | 84,301,440 params (dry-run), FineWeb-Edu BPE-16k m2 shards, 51880x16x1024 = 850,001,920 tok | A100 80GB class (booked GPU/tier/rate recorded at launch, Pravidlo W) | PLANNED | — (fresh G1 input: final_eval_loss on val-eval-2M) | `logs/m2-core-flat-s2-850m-lr6e4.log` (expected) | M2 Phase 3b single permitted retune (docs/cc/M2-phase3b-retune.md); lr 6e-4 (sweep runner-up), dropout 0.0, seed 1337 FIXED — only changed variable vs Phase 3 is lr; AP-24 active; grad_norm logged; step-tagged ckpt mirror; row updated post-run |
| m2-core-ssra-s2-850m-lr6e4 | 2026-07-16 (pre-run row) | `experiments/m2-core-ssra-s2-850m-lr6e4.yaml` (commit 3db45ef) | SSRA P1 | 84,647,040 params (dry-run), ditto | ditto | PLANNED | — (fresh G1 input: final_eval_loss on val-eval-2M) | `logs/m2-core-ssra-s2-850m-lr6e4.log` (expected) | ditto; scoped 30 EUR hard cap (D-log 2026-07-15) enforced in-flight by the early cost gate (`scripts/cost_gate.py`, steps ~1000–1500, ≥200-step window, recomputed at booked rate); row updated post-run |
